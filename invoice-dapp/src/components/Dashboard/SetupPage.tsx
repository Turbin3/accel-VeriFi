...
export function SetupPage({ onComplete }: SetupPageProps) {
  ...
  // Set treasury vault to wallet address
  const deriveTreasuryVault = async () => {
    if (!wallet.publicKey) return;
    // Treasury vault is the wallet address itself, not the token account
    setTreasuryVault(wallet.publicKey.toBase58());
  };
  ...
  const handleCreateOrg = async () => {
    ...
    if (!treasuryVault) {
      setError("Treasury vault not set. Please reconnect wallet.");
      return;
    }
    ...
    try {
      ...
      // Enhanced console logging of all parameters to be passed on-chain
      console.log("Calling orgInit with parameters:");
      console.log("orgConfig PDA:", orgConfigPda.toBase58());
      console.log("authority:", authority.toBase58());
      console.log("treasuryVault:", treasuryVault);
      console.log("mint:", mint);
      console.log("perInvoiceCap (BN):", perInvoiceCapBN.toString());
      console.log("dailyCap (BN):", dailyCapBN.toString());
      console.log("auditRateBps (number):", auditRateBpsNum);
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-slate-700/30 border border-slate-600/50 rounded-lg p-6 space-y-4">
                    ...
                    <div>
                      ...
                      />
                      <p className="text-xs text-slate-500 mt-2">
                        Max amount per single invoice (1B = 1,000 USDC)
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      />
                      <p className="text-xs text-slate-500 mt-2">
                        Max total amount per day (10B = 10,000 USDC)
                      </p>
                    </div>
